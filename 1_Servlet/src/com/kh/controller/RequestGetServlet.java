...
//		System.out.println("잘 실행 됨");
		/*
		 * Get방식으로 요청했기 때문에 doGet메소드가 호출됨
		 * 
		 * 첫번째 매개변수인 HttpServletRequest request에는 요청시 전달된 내용들이 담김(사용자가 입력한 값, 전송방식, 요청한 사용자의 ip 등등)
		 * 
		 * 두번째 매개변수인 HttpServletResponse response에는 요청 처리 후 응답할 때 사용하는 객체가 담김
		 * 
		 ...
		 * - request.getParameter("키값") : String(그에 해당하는 value)값 -> 무조건 문자열로 반환됨
		 * - request.getParameterValues("키값") : String[](그에 해당하는 value값)
		 * => 하나의 key값으로 여러개의 value값을 받는 경우(체크박스) 문자열 배열형으로 반환 가능
		 * */
		...
		if(foods == null) {
			...
			System.out.println("foods : "+String.join(",", foods)); // foods.join(","); -> js에서
			// 배열에 있는 모든 값들을 구분자를 통해서 하나의 문자열로 반환해주는 메소드
		}
		
		// 뽑아낸 값들을 가지고 요청처리 해야 함 (DB와 상호작용)
		// 요청 처리 순서 : Servlet -> Service(뽑아낸 값들을 넘기면서 비지니스로직 실행) -> Dao호출 - DB SQL문 실행 - 결과값 반환
		
		// 위의 과정을 다 처리한 후 사용자가 보게될 응답페이지를 만들어서 전달해주기(순수 Servlet방식)
		/*
		 * 장점 : Java코드 내에 작성하기 때문에 반복문, 조건문, 유용한 메소드를 사용할 수 있다.
		 * 단점 : 복잡하고, 혹시라도 나중에 html수정해야 할 때 Java코드 내에서 수정이 이루어지기 때문에
		 * 		 수정된 내용을 다시 반영시키고자 한다면 서버를 재실행해야함. 사소한 오타로 에러날 수 있음
		 */
		
		// * response객체를 통해 사용자에게 html(응답화면)을 전달
		// 1) 이제부터 내가 출력할 내용은 문서형태이고 html이다. 더불어서 문자셋은 UTF-8이라는 것을 지정
